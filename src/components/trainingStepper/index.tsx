        ...
        <View style={style.container}>
            ...
            <ResizableWhiteCard width="89%" height="72%" borderRadius={10} children={
                <View style={style.midContainer}>
                    ...
                    ?
                    <Image source={{ uri: trainings[actualStep - 1].exercise_url }} style={style.image} />
                    :
                    <Image source={require('../../assets/img/empty_exercise.png')} style={style.image} />
                    }
                    <Text style={style.midTitle}>{trainings[actualStep - 1].exercise_name}</Text>
                    <View style={style.midSub}>
                        <View style={style.midLittleContainers}>
                            <Text style={style.midSubtitle}>Séries</Text>
                            <Text style={style.midNumbers}>{trainings[actualStep - 1].exercise_qtd_serie}</Text>
                        </View>
                        <View style={style.midLittleContainers}>
                            <Text style={style.midSubtitle}>Repetições</Text>
                            <Text style={style.midNumbers}>{trainings[actualStep - 1].exercise_qtd_rep}</Text>
                        </View>
                    </View>
                    <View style={style.midObsContainer}>
                        <Text style={style.midSubtitle}>Observação</Text>
                        <Text style={style.midObs}>{trainings[actualStep - 1].exercise_obs}</Text>
                    </View>
                ...
                {actualStep < trainings.length && actualStep === 1
                    ...
                    actualStep < trainings.length && actualStep > 1
                        ...
                        actualStep === trainings.length && trainings.length === 1
                            ?
                            <TouchableOpacity style={style.singleButton} onPress={() => navigation.navigate('Dashboard')}>
            ...
            <Modal visible={counterIsActive} transparent>
                <TimerModal onclose={() => setCounterIsActive(false)} next={() => actualStep === trainings.length ? navigation.navigate('Dashboard') : nextStep()} time={trainings[actualStep - 1].exercise_rest_time} />
            </Modal>
        </View>
    );
};
